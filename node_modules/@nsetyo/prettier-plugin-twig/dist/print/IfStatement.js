"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.printIfStatement = void 0;
const melody_types_1 = require("melody-types");
const prettier_1 = __importDefault(require("prettier"));
const symbols_1 = require("../symbols");
const util_1 = __importDefault(require("../util"));
const { group, indent, line, hardline } = prettier_1.default.doc.builders;
const { EXPRESSION_NEEDED, printChildBlock } = util_1.default;
const { hasNoNewlines, PRESERVE_LEADING_WHITESPACE, PRESERVE_TRAILING_WHITESPACE, } = util_1.default;
const printIfStatement = (node, path, print) => {
    node[EXPRESSION_NEEDED] = false;
    const hasElseBranch = Array.isArray(node.alternate) && node.alternate.length > 0;
    const hasElseIfBranch = melody_types_1.Node.isIfStatement(node.alternate);
    const isElseIf = node[symbols_1.IS_ELSEIF] === true;
    const isEmptyIf = node.consequent.length === 0;
    const hasOneChild = node.consequent.length === 1;
    const firstChild = node.consequent[0];
    const printInline = !isElseIf &&
        !node.alternate &&
        (isEmptyIf ||
            (hasOneChild &&
                !melody_types_1.Node.isElement(firstChild) &&
                (!melody_types_1.Node.isPrintTextStatement(firstChild) ||
                    hasNoNewlines(firstChild.value.value))));
    // Preserve no-newline white space in single text node child
    if (hasOneChild &&
        melody_types_1.Node.isPrintTextStatement(firstChild) &&
        hasNoNewlines(firstChild.value.value)) {
        firstChild[PRESERVE_LEADING_WHITESPACE] = true;
        firstChild[PRESERVE_TRAILING_WHITESPACE] = true;
    }
    const ifClause = group([
        node.trimLeft ? '{%- ' : '{% ',
        isElseIf ? 'elseif' : 'if',
        indent([line, path.call(print, 'test')]),
        ' ',
        node.trimRightIf ? '-%}' : '%}',
    ]);
    const ifBody = printInline
        ? isEmptyIf
            ? ''
            : path.call(print, 'consequent', '0')
        : printChildBlock(node, path, print, 'consequent');
    const parts = [ifClause, ifBody];
    if (hasElseBranch) {
        parts.push(hardline, node.trimLeftElse ? '{%-' : '{%', ' else ', node.trimRightElse ? '-%}' : '%}');
        parts.push(printChildBlock(node, path, print, 'alternate'));
    }
    else if (hasElseIfBranch) {
        node.alternate[symbols_1.IS_ELSEIF] = true;
        parts.push(hardline);
        parts.push(path.call(print, 'alternate'));
    }
    // The {% endif %} will be taken care of by the "root" if statement
    if (!isElseIf) {
        parts.push(printInline ? '' : hardline, node.trimLeftEndif ? '{%-' : '{%', ' endif ', node.trimRight ? '-%}' : '%}');
    }
    return parts;
};
exports.printIfStatement = printIfStatement;
