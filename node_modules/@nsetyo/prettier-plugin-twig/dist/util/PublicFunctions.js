...
exports.printChildGroups = exports.removeSurroundingWhitespace = exports.isRootNode = exports.isValidIdentifierName = exports.quoteChar = exports.firstValueInAncestorChain = void 0;
const melody_types_1 = require("melody-types");
const prettier_1 = __importDefault(require("prettier"));
const PublicSymbols_1 = require("./PublicSymbols");
const { line, indent, fill, group, hardline } = prettier_1.default.doc.builders;
...
const isContractableNodeType = (node) => {
    for (let i = 0; i < CONTRACTABLE_NODE_TYPES.length; i++) {
        ...
        if (melody_types_1.Node[methodName] && melody_types_1.Node[methodName].call(null, node)) {
            return true;
        }
    }
    if (melody_types_1.Node.isUnaryLike(node)) {
        return true;
    }
    return false;
};
const isNotExpression = (node) => melody_types_1.Node.isUnaryLike(node) && node.operator === 'not';
const isMultipartExpression = (node) => {
    return (melody_types_1.Node.isBinaryExpression(node) ||
        melody_types_1.Node.isConditionalExpression(node) ||
        melody_types_1.Node.isUnaryLike(node));
};
/**
...
exports.firstValueInAncestorChain = firstValueInAncestorChain;
...
const isMelodyNode = (n) => {
    ...
    return (typeof n === 'object' &&
        proto.type &&
        typeof melody_types_1.Node['is' + proto.type] === 'function');
};
...
const shouldExpressionsBeWrapped = (path) => {
    ...
    walkParents(path, (node) => {
        if (node[PublicSymbols_1.INSIDE_OF_STRING] === true) {
            result = PublicSymbols_1.INSIDE_OF_STRING;
            return false;
        }
        if (node[PublicSymbols_1.EXPRESSION_NEEDED] === true) {
            result = PublicSymbols_1.EXPRESSION_NEEDED;
            return false;
        }
        if (node[PublicSymbols_1.EXPRESSION_NEEDED] === false ||
            node[PublicSymbols_1.INSIDE_OF_STRING] === false) {
...
const wrapExpressionIfNeeded = (path, fragments, node = {}) => {
    ...
    if (wrapType === PublicSymbols_1.EXPRESSION_NEEDED) {
        wrapInEnvironment(fragments, node.trimLeft, node.trimRight);
    }
    else if (wrapType === PublicSymbols_1.INSIDE_OF_STRING) {
...
const unifyWhitespace = (s, replacement = ' ') => splitByWhitespace(s)
...
const isWhitespaceNode = (node) => {
    return ((melody_types_1.Node.isPrintTextStatement(node) &&
        isWhitespaceOnly(node.value.value)) ||
        (melody_types_1.Node.isStringLiteral(node) && isWhitespaceOnly(node.value)));
};
const isEmptySequence = (node) => melody_types_1.Node.isSequenceExpression(node) && node.expressions.length === 0;
...
exports.removeSurroundingWhitespace = removeSurroundingWhitespace;
...
const printChildBlock = (node, path, print, ...childPath) => {
    ...
    setDeepProperty(node, (0, exports.removeSurroundingWhitespace)(originalChildren), ...childPath);
    const childGroups = (0, exports.printChildGroups)(node, path, print, ...childPath);
...
const isInlineElement = (node) => {
    const isInlineHtmlElement = melody_types_1.Node.isElement(node) && INLINE_HTML_ELEMENTS.indexOf(node.name) >= 0;
    return (isInlineHtmlElement ||
        melody_types_1.Node.isPrintExpressionStatement(node) ||
        isInlineTextStatement(node));
};
const isCommentNode = (node) => melody_types_1.Node.isTwigComment(node) || melody_types_1.Node.isHtmlComment(node);
const createInlineMap = (nodes) => nodes.map((node) => isInlineElement(node));
const textStatementsOnlyNewlines = (nodes) => {
    nodes.forEach((node) => {
        if (melody_types_1.Node.isPrintTextStatement(node)) {
            node[PublicSymbols_1.NEWLINES_ONLY] = true;
        }
    });
};
    ...
    nodes.forEach((node, index) => {
        ...
        if (melody_types_1.Node.isPrintTextStatement(node)) {
            ...
            if (hasPreviousInlineElement || previousNodeIsComment) {
                node[PublicSymbols_1.PRESERVE_LEADING_WHITESPACE] = true;
            }
            ...
            if (hasFollowingInlineElement || followingNodeIsComment) {
                node[PublicSymbols_1.PRESERVE_TRAILING_WHITESPACE] = true;
            }
        }
    });
};
...
exports.printChildGroups = printChildGroups;
exports.default = {
    ...
    wrapInEnvironment,
    findParentNode,
    isRootNode: exports.isRootNode,
    isMelodyNode,
    someParentNode,
    walkParents,
    firstValueInAncestorChain: exports.firstValueInAncestorChain,
    isContractableNodeType,
    isNotExpression,
    isMultipartExpression,
    registerContractableNodeType,
    quoteChar: exports.quoteChar,
    isValidIdentifierName: exports.isValidIdentifierName,
    testCurrentNode,
    ...
    createTextGroups,
    removeSurroundingWhitespace: exports.removeSurroundingWhitespace,
    getDeepProperty,
    setDeepProperty,
    isInlineElement,
    printChildBlock,
    printChildGroups: exports.printChildGroups,
    indentWithHardline,
};
