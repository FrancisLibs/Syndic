...
class CharStream {
    ...
    la(offset) {
        ...
        return index < this.length ? this.input.charAt(index) : symbols_1.EOF;
    }
    lac(offset) {
        const index = this.index + offset;
        return index < this.length ? this.input.charCodeAt(index) : symbols_1.EOF;
    }
    next() {
        if (this.index === this.length) {
            return symbols_1.EOF;
        }
    ...
    match(str) {
        ...
        for (let i = 0, len = str.length; i < len; i++) {
            const ch = this.next();
            if (ch === symbols_1.EOF || ch !== str.charAt(i)) {
                this.rewind(start);
                return false;
            }
        }
