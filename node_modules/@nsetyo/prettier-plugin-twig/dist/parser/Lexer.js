...
const TokenTypes = __importStar(require("./TokenTypes"));
const State = {
    TEXT: 'TEXT',
...
class Lexer {
    constructor(input, { preserveSourceLiterally = false } = {}) {
        this[symbols_1.STATE] = [State.TEXT];
        this[symbols_1.OPERATORS] = [];
        this[symbols_1.STRING_START] = null;
        this.input = input;
        this.options = {
    ...
    addOperators(...ops) {
        this[symbols_1.OPERATORS].push(...ops);
        this[symbols_1.OPERATORS].sort((a, b) => (a.length > b.length ? -1 : 1));
    }
    get state() {
        return this[symbols_1.STATE][this[symbols_1.STATE].length - 1];
    }
    pushState(state) {
        this[symbols_1.STATE].push(state);
    }
    popState() {
        this[symbols_1.STATE].length--;
    }
    ...
    next() {
        ...
        let pos;
        let c;
        while ((c = input.la(0)) !== symbols_1.EOF) {
            pos = input.mark();
            if (this.state !== State.TEXT &&
                ...
                isWhitespace(c)) {
                input.next();
                while ((c = input.la(0)) !== symbols_1.EOF && isWhitespace(c)) {
                    input.next();
                }
                    ...
                    if (input.la(1) === '{' ||
                        ...
                        while ((c = input.la(0)) !== symbols_1.EOF) {
                            if (c === '-' && input.la(1) === '-') {
                                ...
                                input.next();
                                if (!((c = input.next()) === '>')) {
                                    this.error('Unexpected end for HTML comment', input.mark(), `Expected comment to end with '>' but found '${String(c)}' instead.`);
                                }
                                break;
                            }
                            input.next();
            ...
            else if (this.state === State.STRING_SINGLE ||
                this.state === State.STRING_DOUBLE) {
    ...
    matchExpression(pos) {
        ...
        switch (c) {
            ...
            default: {
                ...
                const { longestMatchingOperator, longestMatchEndPos } = this.findLongestMatchingOperator();
    ...
    findLongestMatchingOperator() {
        ...
        let longestMatchEndPos = {};
        const ops = this[symbols_1.OPERATORS];
    ...
    matchSymbol(pos) {
        ...
        if (pos.index === end.index) {
            return this.error('Expected an Identifier', pos, inElement
                ? `Expected a valid attribute name, but instead found "${input.la(0)}", which is not part of a valid attribute name.`
                : `Expected letter, digit or underscore but found ${input.la(0)} instead.`);
        }
    ...
    matchString(pos, allowInterpolation = true) {
        ...
        while ((c = input.la(0)) !== start && c !== symbols_1.EOF) {
    ...
    matchNumber(pos) {
        const input = this.input;
        let c;
        while ((c = input.lac(0)) !== symbols_1.EOF) {
            if (!isDigit(c)) {
                break;
            }
            input.next();
    ...
    matchText(pos) {
        const input = this.input;
        let c;
        while ((c = input.la(0)) && c !== symbols_1.EOF) {
            if (c === '{') {
                const c2 = input.la(1);
                if (c2 === '{' || c2 === '#' || c2 === '%') {
                    break;
    ...
    matchComment(pos) {
        const input = this.input;
        let c;
        while ((c = input.next()) !== symbols_1.EOF) {
            if (c === '#' && input.la(0) === '}') {
                input.next(); // consume '}'
                break;
            }
...
function isWhitespace(c) {
    return c !== symbols_1.EOF && (c === '\n' || c === ' ' || c === '\t');
}
function isAlpha(c) {
    return c !== symbols_1.EOF && ((65 <= c && c <= 90) || (97 <= c && c <= 122));
}
function isDigit(c) {
    return c !== symbols_1.EOF && 48 <= c && c <= 57;
}
